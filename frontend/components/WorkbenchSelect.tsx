...
export default function WorkbenchSelect({ className = "", children, ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return <span className={`relative inline-flex min-w-0 ${className}`}>
    <select {...props} className="w-full cursor-pointer appearance-none rounded-xl border border-line-strong bg-panel py-2.5 pl-3 pr-9 text-sm text-ink shadow-sm outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20 disabled:cursor-not-allowed disabled:opacity-40">{children}</select>
