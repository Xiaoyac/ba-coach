"""Guarded activation of the natural-consent / empty-reply recovery patch.

This script runs on the BA Coach server and accepts only the six files in
``route-empty-0919/candidate/patch.tar.gz``. It builds a new release from the
...
Usage on the server::
    ...
    python3 deploy_consent_reply_0919.py prepare RELEASE_ID ARCHIVE VERIFIER
    python3 deploy_consent_reply_0919.py activate RELEASE_ID ARCHIVE VERIFIER

``VERIFIER`` is a small local script path on the server.  The script never
mutates or migrates a database, copies a knowledge base, or changes the
...
PREVIOUS = ROOT / "releases/20260918T195630Z"
ALLOWED = {
    "backend/app/m1_contract.py",
    "backend/app/graph/nodes.py",
    "backend/app/prompts.py",
    "backend/app/providers/deepseek.py",
    "backend/app/reply_recovery.py",
...
def switch(path: Path, release_id: str) -> None:
    link = ROOT / (".consent-reply-switch-" + release_id)
...
print("PREFLIGHT_OK: prior memory repair preserved; safety gates enabled; database maintenance disabled",flush=True)
