...
模块1（BA心理教育）：建立信任、收集困扰、拆解抑郁循环、科普BA基础知识。
...
1. 跳转至模块二：
- current_module=1；完整对话中已经获取到足以形成行为—情绪循环的困扰核心内容（不需要穷尽全部细节）、已经结合用户经历讲解抑郁循环与 BA 基础知识、用户已经明确表达理解并愿意开始目标设定，以上三项必须全部达成。满足后应让下一轮进入模块二；不要求模块一先越权开展具体目标设定；
