...
async def schedule_check(db, caller, device_id, *, at=None):
    ...
    row = {'id': ident, 'user_id': caller.subject_id, 'device_id': device_id, 'state': 'queued',
        'due_at': naive_utc(current + timedelta(seconds=60)), 'expires_at': naive_utc(current + timedelta(minutes=11)),
...
async def record_receipt(db, ident, token, had_open_window, *, at=None):
    ...
    if (not row or not row['receipt_digest'] or not secrets.compare_digest(row['receipt_digest'], digest)
            or utc(row['expires_at']) <= current or row['state'] not in {'attempting', 'accepted', 'unknown'}):
...
async def tick_checks(sessionmaker, *, at=None, sender=send_push):
    ...
    for row in pending:
        ...
        try:
            code = await asyncio.to_thread(sender, device, payload, max(1, int((utc(row['expires_at']) - send_at).total_seconds())))
        except Exception:
